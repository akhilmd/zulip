...
function send_message(skip_correct_narrow_check, request) {
    ...
    if (!skip_correct_narrow_check && narrow_state.active()) {
        ...
        if (request.type === "stream") {
            ...
            args.narrow_to_text = (request.to_user_ids.indexOf(",") < 0 ? "" : i18n.t("group ")) +
                i18n.t("PM with ") + request.private_message_recipient;
...
exports.initialize = function () {
    ...
    $("#send-status").on("click", "#outside-narrow-send-button", function () {
        send_message(true);
        $("#send-status").hide();
    });
